...
<head>
    ...
    <title>Date wise Cashier report List</title>
...
<body>
    <header class="clearfix">
        ...
        @endif
        <div class="info">
            <h2><span>Date wise Cashier report List</span></h2>
            <h5>
    ...
    <main>
        ...
        <table class="order-details">
            <thead>
                <tr class="bg-blue">
                    ...
                    <th width="25%" class="text-center">R.Date</th>
                    <th width="25%" class="text-center">EMP.Info</th>
                    <th width="25%" class="text-center">Inv.Info</th>
                    <th width="20%" class="text-center">R.Amount</th>
                </tr>
            </thead>
            <tbody>
                @if (count($invoice_details_arr) > 0)
                    @foreach ($invoice_details_arr as $key => $data)
                        <tr>
                            ...
                            <td class="text-center">{{ date('d F Y', strtotime($data->receive_date)) }}</td>
                            <td>
                                <strong>Card No</strong>:
                                {{ $data->card_no && $data->card_no != '' ? $data->card_no : '' }}<br>
                                <strong>Emp</strong>: {{ $data->name && $data->name != '' ? $data->name : '' }}
                            </td>
                            <td>
                                {{ date('d F Y', strtotime($data->inv_date)) }}<br>
                                <strong>Inv. No </strong>:
                                {{ $data->invoice_no && $data->invoice_no != '' ? $data->invoice_no : '' }} <br>
                                <strong>Store </strong>:
                                {{ $data->store_name && $data->store_name != '' ? $data->store_name : '' }}
                            </td>
                            <td class="text-center">
                                {{ $data->receive_amount && $data->receive_amount != '' ? $data->receive_amount : '0' }}
                            </td>
                        </tr>
                        @php
                            $total_receive_amount = $total_receive_amount + ($data->receive_amount ? $data->receive_amount : '0');
                        @endphp
                    @endforeach
                @else
                    <tr>
        ...
        <table style="border: 0px;">
            <table class="four_table" style="border: 0px;">
                <tbody>
                    <tr>
                        <td style="border: 0px;"><b>Total Receive Amount </b></td>
                        <td style="border: 0px;">:&nbsp; {{ $total_receive_amount }}</td>
                    </tr>
                </tbody>
            </table>
        </table>
