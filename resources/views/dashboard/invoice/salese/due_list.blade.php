                    ...
                    <section class="col-lg-12 connectedSortable">
                        <div class="card">
                            <div class="card-header">
                                <div class="">
                                    <form action="{{ route('admin.due_print_invoice_list') }}" target="_blank"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                type: "get",
                dataType: "json",
                url: "{{ route('admin.due_invoice_list_show') }}",
                beforeSend: function() {
                    uiBlock()
                },
        ...
        let showData = async (res) => {
            ...
            if (res.length != 0) {
                ...
                $.each(res, function(key, value) {
                    ...
                    let due_collection_url = "{{ route('admin.due_collection', ':id') }}";
                    due_collection_url = due_collection_url.replace(':id', value.invoice_id);

                    let cancel_invoice = "{{ route('admin.cancel_invoice', ':id') }}";
                    cancel_invoice = cancel_invoice.replace(':id', value.invoice_id);

                    let due_invoice_details = "{{ route('admin.due_invoice_details', ':id') }}";
                    due_invoice_details = due_invoice_details.replace(':id', value.invoice_id);

                    let due_invoice_customer_copy_view =
                        "{{ route('admin.due_invoice_customer_copy_view', ':id') }}";
                    due_invoice_customer_copy_view = due_invoice_customer_copy_view.replace(':id', value
                        .invoice_id);
                    ...
                    $.each(value.payment_details, function(key, pay) {
                        ...
                        pay_data +=
                            `<a class="btn-sm px-2  mb-2 btn-info open_modal" type="button" data-action="${invoice_per_payment}" data-title="Payment Info (Pay-${j}) (Invoice : ${value.invoice_no}) (Date : ${d.getDate()+' '+(d.toLocaleString('default', { month: 'long' }))+' '+d.getFullYear()})" data-modal="common_modal_xl" title="Payment Info">Pay-${j}</a>  &nbsp;&nbsp;`;
                    })
                            ...
                            <tr>
                                ...
                                <td scope="col" class="text-left">
                                 ...
                                 <td scope="col" class="text-center">
                                    <a type='button' href="${due_collection_url}" class="btn-sm btn-primary mb-2" > Due Collect </a>&nbsp;
                                    <a type='button' data-action="${due_invoice_details}" class="btn-sm btn-secondary mb-2 text-white open_modal" data-title="Invoice Details  (Invoice no. : ${value.invoice_no}) (Date : ${d.getDate()+' '+(d.toLocaleString('default', { month: 'long' }))+' '+d.getFullYear()})" data-modal="common_modal_xl" title="Invoice Details"> View</a> <br>
                                    <a type='button' data-action="${due_invoice_customer_copy_view}" class="btn-sm btn-warning mb-2 text-white open_modal" data-title="Invoice Info  (Invoice no. : ${value.invoice_no}) (Date : ${d.getDate()+' '+(d.toLocaleString('default', { month: 'long' }))+' '+d.getFullYear()})" data-modal="common_modal_xl" title="Invoice Info"> C.Copy</a> &nbsp;
        ...
        $(".search_btn").click(function() {
            ...
            $.ajax({
                ...
                url: "{{ route('admin.due_search_invoice_list') }}",
                beforeSend: function() {
                    uiBlock()
                },
                success: function(res) {
